// $Id: packet.c,v 1.35 2020/03/09 06:07:26 karn Exp $
...
int const AL = 960; // 24 bit times
int const AM = 1049;
float const Samprate = 48000;
float const Bitrate = 1200;
#define Samppbit (Samprate/Bitrate)
...
struct option Options[] = { { "iface", required_argument, NULL, 'A' }, {
		"pcm-in", required_argument, NULL, 'I' }, { "ax25-out",
		required_argument, NULL, 'R' }, { "status-in", required_argument, NULL,
		'S' }, { "ttl", required_argument, NULL, 'T' }, { "verbose",
		no_argument, NULL, 'v' }, { NULL, 0, NULL, 0 }, };
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt_long(argc, argv, Optstring, Options, NULL)) != EOF) {
		...
		case 'I':
			...
			if (Status_fd != -1)
				fprintf(stderr,
						"warning: --status-in ignored when --pcm-in specified\n");
			break;
		case 'R':
		...
		case 'S':
			if (Nfds != 0) {
				fprintf(stderr,
						"--status-in ignored when --pcm-in specified\n");
				break;
			}
			if (Status_fd != -1) {
			...
			Status_fd = setup_mcast(optarg,
					(struct sockaddr*) &Status_dest_address, 0, 0, 2);
			if (Status_fd == -1) {
		...
		default:
			fprintf(stderr,
					"Usage: %s [--verbose|-v] [--ttl|-T mcast_ttl] [--pcm-in|-I input_mcast_address [--pcm-in|-I address2]] [--ax25-out|-R output_mcast_address] [input_address ...]\n",
					argv[0]);
			exit(1);
		}
	}
	...
	while (1) {
		...
		int length = recvfrom(Status_fd, buffer, sizeof(buffer), 0,
				(struct sockaddr*) &Status_input_source_address, &socklen);

		// We MUST ignore our own status packets, or we'll loop!
		if (memcmp(&Status_input_source_address, &Local_status_source_address,
				sizeof(Local_status_source_address)) == 0)
			continue;
			...
			while (cp - buffer < length) {
				...
				case OUTPUT_DATA_DEST_SOCKET:
					...
					if (Nfds == 0) {
						...
						Input_fd[Nfds] = setup_mcast(NULL,
								(struct sockaddr*) &PCM_dest_address, 0, 0, 0);
						if (Input_fd[Nfds] != -1) {
							Max_fd = max(Max_fd, Input_fd[Nfds]);
							FD_SET(Input_fd[Nfds], &Fdset_template);
							Nfds++;
...
void* input(void *arg) {
	...
	while (1) {
		...
		for (int fd_index = 0; fd_index < Nfds; fd_index++) {
			...
			int size = recvfrom(Input_fd[fd_index], buffer, sizeof(buffer), 0,
					&sender, &socksize);
			if (size == -1) {
			...
			if (sp == NULL) {
				...
				if (Verbose) {
					...
					fprintf(stdout, "New session from %s:%s, ssrc %x\n",
							sp->source.host, sp->source.port,
							sp->rtp_state_in.ssrc);
					fflush(stdout);
				}
			}
			int sample_count = size / sizeof(signed short); // 16-bit sample count
			int skipped_samples = rtp_process(&sp->rtp_state_in, &rtp_hdr,
					sample_count);
			if (skipped_samples < 0)
				continue;	// Drop probable duplicate(s)

			// Keep sample count correct when packets are dropped
			while (skipped_samples-- > 0) {
				sp->filter_in->input.r[sp->input_pointer++] = 0;
			...
			while (sample_count-- > 0) {
				...
				sp->filter_in->input.r[sp->input_pointer++] = ntohs(*samples++)
						* SCALE;
				if (sp->input_pointer == sp->filter_in->ilen) {
					execute_filter_input(sp->filter_in); // Wakes up any threads waiting for data on this filter
					sp->input_pointer = 0;
...
void* decode_task(void *arg) {
	const float twist = 1200. / 2200.; // Scale back upper tone from FM demod

	pthread_setname("afsk");
	struct session *sp = (struct session*) arg;
	assert(sp != NULL);

	struct filter_out *filter = create_filter_output(sp->filter_in, NULL, 1,
			COMPLEX);
	...
	int symphase = 0;
	float complex
	mark_accum = 0; // On-time
	float complex
	space_accum = 0;
	float complex
	mark_offset_accum = 0; // Straddles previous zero crossing
	float complex
	space_offset_accum = 0;
	float last_val = 0;  // Last on-time symbol
	float mid_val = 0;   // Last zero crossing symbol

	// hdlc state
	unsigned char hdlc_frame[16384];
	...
	while (1) {
		...
		for (int n = 0; n < filter->olen; n++) {
			...
			if (++symphase == Samppbit / 2) {
				...
				mid_val = cnrmf(mark_offset_accum)
						- twist * cnrmf(space_offset_accum);
			...
			if (cur_val * last_val >= 0) {
				...
				if (++ones == 7) {
					...
					frame_bit++;
					if ((frame_bit << 3) > sizeof(hdlc_frame)) {
						// Too long; abort
						memset(hdlc_frame, 0, sizeof(hdlc_frame));
						frame_bit = 0;
						flagsync = 0;
				...
				if (ones < 5 && flagsync) {
					frame_bit++;
				} else if (ones == 6) { // Note: ones == 5 case not handled; drop stuffed zero
					if (flagsync) {	  // Flag already seen
						int bytes = (frame_bit - 7) / 8; // Don't count leading 0111111
						if (bytes > 0 && crc_good(hdlc_frame, bytes)) {
							if (Verbose) {
								...
								fprintf(stdout,
										"%d %s %04d %02d:%02d:%02d UTC ",
										tmp->tm_mday, Months[tmp->tm_mon],
										tmp->tm_year + 1900, tmp->tm_hour,
										tmp->tm_min, tmp->tm_sec);

								fprintf(stdout, "ssrc %x packet %d len %d:\n",
										sp->rtp_state_in.ssrc,
										sp->decoded_packets++, bytes);
